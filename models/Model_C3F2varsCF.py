...
def load_data(fname):
    ...
    limits = np.array([[0,400],[400,500],[500,750],[750,1000],[400-170,400-90],[400-170,400]])
    ...
    X_train = list()
    X_train.append(np.array([x[limits[0][0]:limits[0][1]] for x in Xh_train]))
    X_train.append(Xv_train[:,8:])
    X_test = list()
    X_test.append(np.array([x[limits[0][0]:limits[0][1]] for x in Xh_test]))
